...
class JFormFieldSYWIconPicker extends JFormField
{
	...
	protected function getInput()
	{
		...
		$script = 'jQuery(document).ready(function () { ';
			...
			$script .= 'jQuery("#'.$this->id.'_select li[data-SYWicon]").click(function() { ';
				// de-select the previous value
				$script .= 'jQuery("#'.$this->id.'_select li[data-SYWicon] a").each(function() { ';
			...
			$script .= 'jQuery("#'.$this->id.'_default").click(function() { ';
				...
				if ($this->buttonrole == 'default') {
					$script .= 'jQuery(this).addClass("btn-primary"); ';
				}
				$script .= 'jQuery("#'.$this->id.'_select li[data-SYWicon] a").removeClass("label-success"); ';
			$script .= '}); ';

			if ($this->editable) {
    			$script .= 'jQuery("#'.$this->id.'").change(function() { ';
    				$script .= 'jQuery("#' . $this->id . '_icon").attr("class", "SYWicon-" + jQuery(this).val()); ';
			...
			$script .= 'jQuery(document).on("subform-row-add", function(event, row) { ';
                $script .= 'jQuery(row).find(".iconpicker li[data-SYWicon]").each(function() { ';
			        $script .= 'jQuery(this).click(function() { ';     
                        $script .= 'var inputval = jQuery(row).find(".iconpicker input[data-name=input-icon]").val(); ';
                        $script .= 'if (inputval != "") { jQuery(row).find(".iconpicker li[data-SYWicon=" + inputval + "] a").removeClass("label-success"); } ';
                        $script .= 'jQuery(row).find(".iconpicker input[data-name=input-icon]").val(jQuery(this).attr("data-SYWicon")); ';
			            $script .= 'jQuery(row).find(".iconpicker i[data-name=icon]").attr("class", "SYWicon-" + jQuery(this).attr("data-SYWicon")); ';
			            if ($this->buttonrole == 'default') {
			                $script .= 'jQuery(row).find(".iconpicker a[data-name=default-icon]").removeClass("btn-primary"); ';
			            }
			            $script .= 'jQuery(this).children(":first").addClass("label-success"); ';
			        $script .= '}); ';
			    $script .= '}); ';
			    $script .= 'jQuery(row).find(".iconpicker a[data-name=default-icon]").click(function() { ';
    			    if (empty($this->default)) {
    			        $script .= 'jQuery(row).find(".iconpicker input[data-name=input-icon]").val(""); ';
    			        $script .= 'jQuery(row).find(".iconpicker i[data-name=icon]").attr("class", "SYWicon-'.$this->emptyicon.'"); ';
    			    } else {
    			        $script .= 'jQuery(row).find(".iconpicker input[data-name=input-icon]").val("'.$this->default.'"); ';
    			        $script .= 'jQuery(row).find(".iconpicker i[data-name=icon]").attr("class", "SYWicon-'.$this->default.'"); ';
    			    }	
    			    if ($this->buttonrole == 'default') {
    			        $script .= 'jQuery(this).addClass("btn-primary"); ';
    			    }
    			    $script .= 'jQuery(row).find(".iconpicker li[data-SYWicon] a").removeClass("label-success"); ';
			    $script .= '}); ';
			    if ($this->editable) {
			        $script .= 'jQuery(row).find(".iconpicker input[data-name=input-icon]").change(function() { ';
                        $script .= 'jQuery(row).find(".iconpicker i[data-name=icon]").attr("class", "SYWicon-" + jQuery(this).val()); ';                        
                        $script .= 'jQuery(row).find(".iconpicker li[data-SYWicon] a").removeClass("label-success"); ';                        
                        $script .= 'jQuery(row).find(".iconpicker li[data-SYWicon=" + jQuery(this).val() + "] a").addClass("label-success"); ';
                        if ($this->buttonrole == 'default') {
                            $script .= 'jQuery(row).find(".iconpicker a[data-name=default-icon]").removeClass("btn-primary"); ';
                        }
			        $script .= '}); ';
		...
		$html .= '<div class="iconpicker input-prepend input-append">';

 		if (!empty($this->value)) {
 			$html .= '<div class="add-on"><i id="'.$this->id.'_icon" class="SYWicon-'.$this->value.'" data-name="icon" aria-hidden="true"></i></div>';
 		} else {
			if (empty($this->default)) {
				$html .= '<div class="add-on"><i id="'.$this->id.'_icon" class="SYWicon-'.$this->emptyicon.'" data-name="icon" aria-hidden="true"></i></div>';
			} else {
				$html .= '<div class="add-on"><i id="'.$this->id.'_icon" class="SYWicon-'.$this->default.'" data-name="icon" aria-hidden="true"></i></div>';
			}
 		}

 		if ($this->editable) {
			$html .= '<input type="text" name="'.$this->name.'" id="'.$this->id.'"'.' data-name=input-icon value="'.htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8').'" class="input-small" />';
		} else {
			//$html .= '<input type="hidden" name="'.$this->name.'" id="'.$this->id.'"'.' value="'.htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8').'" />';
			$html .= '<input type="text" name="'.$this->name.'" id="'.$this->id.'"'.' data-name=input-icon value="'.htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8').'" readonly="readonly" class="input-small" />';
		}
		...
		if (isset($this->icons)) {
			...
			foreach ($icons as $icon_item) {
				$html .= '<li style="width: auto; float: left; margin: 2px;" data-SYWicon="'.$icon_item.'"><a href="#" class="label hvr-grow hasTooltip" style="padding: 8px; color: #fff; font-size: 1.4em" title="'.$icon_item.'" aria-label="'.$icon_item.'" onclick="return false;"><i class="SYWicon-'.$icon_item.'"></i></a></li>';
			}
		...
		$html .= '<a data-name=default-icon id="'.$this->id.'_default"'.($this->disabled ? ' disabled="disabled"' : '').' class="btn'.$default_class_extra.' hasTooltip" title="'.htmlspecialchars($this->buttonlabel, ENT_COMPAT, 'UTF-8').'" href="#" aria-label="' . htmlspecialchars($this->buttonlabel, ENT_COMPAT, 'UTF-8') . '" onclick="return false;">';
